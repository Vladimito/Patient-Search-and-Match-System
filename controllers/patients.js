...
const updateOne = async(req, res) =>{
	...
	let db = req.db;
	try{
		console.log("pUpdate: " + JSON.stringify(pUpdate));
		let msg = await Patient.update(db,patientID,pUpdate.id,pUpdate.bday,pUpdate.dday,pUpdate.ssn,pUpdate.passport,pUpdate.prefix,pUpdate.first,pUpdate.last,pUpdate.suffix,pUpdate.maiden,pUpdate.marital,pUpdate.race,pUpdate.ethnicity,pUpdate.gender,pUpdate.birthplace,pUpdate.address,pUpdate.city,pUpdate.county,pUpdate.zip,pUpdate.lat,pUpdate.lon,pUpdate.healthExpenses,pUpdate.healthCoverage);
		res.send(msg);
	}catch(err){
...
const deleteOne = async (req, res) => {
	...
	try{
		...
		res.send('There was an error while deleting your Book. (err:'+JSON.stringify(err.msg)+')');
		throw new Error(JSON.stringify(err.msg));
	}		
}
